...
var app = angular.module("app", ["duScroll", "toggle-switch", "ngSocial", "myNav"]);
...
app.controller("MainCtrl", function($scope, $http) {
    ...
    $scope.main.musicEnabled = true;
    $scope.main.progress = 0;
    var allItems;
    ...
    $scope.sndClick = new Howl({
        urls: ['assets/sndClick.mp3']
    });
    $scope.sndMusic = new Howl({
        urls: ['assets/music.mp3'],
        loop:true
    });
    //$scope.sndMusic.play();

    $scope.$watch("main.musicEnabled", function (newValue) {
    ...
    $scope.nextLevel = function () {
        ...
        $scope.main.progress = $scope.main.levelIndex / allItems.length;
...
app.directive("nextButton", function () {
    return {
        link: function (scope, el, attrs) {
            el.addClass("btn btn-success btn-lg btn-block bt-next");
...
app.directive("sndClick", function () {
    return {
        controller: function ($scope) {
...
app.directive("myBar", function() {
    return {
        link:function(scope, el, attrs) {
            scope.$watch("main.progress", function(newValue, oldValue) {
                el.css({
                    width:newValue * 100 + "%"
                })
            })
